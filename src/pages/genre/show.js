...
const ShowGenrePage = () => {
  ...
  const fetchShows = async (pageNum) => {
    try {
      const response = await axios.get(
        `https://api.themoviedb.org/3/discover/tv?api_key=${API_KEY}&with_genres=${id}&language=en-US&page=${pageNum}`
      );

      if (response.data.results.length === 0) {
        setHasMore(false); // No more pages to load
      } else {
        setShows((prev) => [...prev, ...response.data.results]); // Append new data
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (id) {
      setShows([]); // Reset when genre changes
      setPage(1);
      setHasMore(true);
      fetchShows(1);
    }
  ...
  return (
    <>
      <NextSeo
        ...
        description={`Dive into the world of ${title}! Explore a collection of the best ${title} tv shows, from timeless classics to the latest releases. Experience stunning visuals and captivating stories for all ages!`}
        ...
        openGraph={{
          ...
          url: `https://prime-vision.vercel.app/genre/show?id=${id}&title=${title}`,
          images: [
            {
      ...
      />
      <div className="px-2">
        <h1 className="text-2xl font-bold mb-6">TV Shows - {title}</h1>
        <div className="flex gap-6 flex-wrap justify-around">
          {shows.map((item, index) => (
            <Link href={`/watch/show?id=${item.id}`} key={index}>
              <div
                ref={index === shows.length - 1 ? lastElementRef : null} // Observe last item
